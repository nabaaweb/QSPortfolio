...
for (let i = 0; i < twitter.length; i++) {
  twitter[i].addEventListener("click", function () {
    window.location.href = "https://x.com/kassemsaad";
  });
}
...
let skill = document.querySelectorAll(".skills .cards .more");
let viewSkillsBtn = document.querySelector(".view-more-skills");
viewSkillsBtn.onclick = function () {
  for (let i = 0; i < skill.length; i++) {
    skill[i].classList.toggle("close");
  }
  if (skill[0].classList.contains("close")) viewSkillsBtn.innerHTML = "View More";
  else {
    viewSkillsBtn.innerHTML = "View Less";
  }
};